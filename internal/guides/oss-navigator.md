...
1. **Topology Discovery**:
   - Query `trace_cross_service_flow(query="...")` on `oss-ask` or `get_related_repos(repo_name="...")` on `oss-indexer`.
...
2. **AST Graph Tracing**:
   ...
   - Read exact code signatures with `get_code_snippet`.

3. **Synthesis Styles**:
   - **Targeted Code Inquiry**: Provide exact file links, function signatures, validation logic, and DTO structures.
   - **End-to-End Flow**: Provide step-by-step hops (Client -> Route -> Auth Middleware -> Controller -> Database / Event) with a concise Mermaid sequence diagram.
